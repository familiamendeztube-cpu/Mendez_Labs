...
import { Sparkles, X, ArrowUp, Loader2, Trash2 } from 'lucide-react';
import { askCopilot, type CopilotMessage } from '@/services/copilot';
...
export function Copilot() {
  ...
  const send = useCallback(async (text: string) => {
    ...
    setBusy(true);
    try {
      const reply = await askCopilot(next, getContext());
  ...
  return (
    <>
      {/* Floating trigger — clears the mobile bottom nav */}
      {!open && (
        <button
          ...
          style={{ background: tv.accent, color: tv.bgOverlay, boxShadow: '0 12px 32px rgba(0,0,0,0.4)' }}
        >
      ...
      {open && (
        <>
          ...
          />
          <div
            className="fixed z-[70] flex flex-col overflow-hidden rounded-2xl border shadow-2xl inset-x-3 bottom-3 top-16 lg:inset-x-auto lg:top-auto lg:bottom-6 lg:right-6 lg:h-[600px] lg:w-[400px]"
          ...
          >
            ...
            <div className="flex items-center justify-between border-b px-4 py-3" style={{ borderColor: tv.borderBase }}>
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4" style={{ color: tv.accent }} />
                <span className="text-sm font-semibold" style={{ color: tv.textPrimary }}>Copilot</span>
                <span className="text-[10px] font-medium uppercase tracking-wider" style={{ color: tv.textMuted }}>advice only</span>
              </div>
            ...
            <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
              {messages.length === 0 && (
                <div className="space-y-3">
                  <p className="text-sm leading-relaxed" style={{ color: tv.textSecondary }}>
                    I can see your live Alpaca account, today's picks, and your bankroll. Ask me anything about your trades or bets.
                  </p>
              ...
              {messages.map((m, i) => (
                <div
                  ...
                  style={
                    m.role === 'user'
                      ? { marginLeft: 'auto', background: tv.accent, color: tv.bgOverlay }
              ...
              {busy && (
                <div className="flex items-center gap-2 rounded-2xl px-3.5 py-2.5" style={{ background: tv.bgElevated, border: `1px solid ${tv.borderBase}`, width: 'fit-content' }}>
            ...
            >
              <textarea
                ...
                rows={1}
                placeholder="Ask about your trades or bets…"
